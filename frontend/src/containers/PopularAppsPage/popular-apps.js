...
function PopularAppsController (
  ...
  function getPopularApps() {
    const params = convertTagsToParams();
  ...
  function convertTagsToParams() {
    const params = {
      'countries[]': [], 'platforms[]': [], 'categories[]': [], 'rank_types[]': [],
    };
  ...
  function enrichApps() {
    for (const app of popularApps.apps) {
  ...
  function trackSortingClick(sort, order) {
    mixpanel.track('Sorted Popular Apps', Object.assign({ sort, order }, mixpanelQueryData()));
  }
