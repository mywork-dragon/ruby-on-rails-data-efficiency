...
export const isFacebookOnly = state => (accessibleNetworks(state).length === 1 && accessibleNetworks(state)[0].id === 'facebook') || accessibleNetworks(state).length === 0;

export const needPermissions = state => !state.account.permissions.loaded && !state.account.permissions.fetching;

export const canAccessFeature = (state, feature) => state.account.permissions.permissions.features && state.account.permissions.permissions.features[feature];
