class IosReclassificationService
  class << self
    def reclassify_current_newest
      apps = IosApp.joins(:ipa_snapshots).select(:id).distinct.where('ipa_snapshots.success = true').where("ipa_snapshots.scan_status = #{IpaSnapshot.scan_statuses[:scanned]}").count
