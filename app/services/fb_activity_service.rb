class FbActivityService
  class << self
    ...
    def simulate
      ...
      Slackiq.notify(webhook_name: :main, title: 'Facebook Activity Complete', accounts: FbAccount.count, successes: job.fb_activities.count, failures: job.fb_activity_exceptions.count, likes: job.fb_activities.pluck(:likes).inject(:+), statuses: job.fb_activities.where.not(status: nil).count)
    end
  end
end