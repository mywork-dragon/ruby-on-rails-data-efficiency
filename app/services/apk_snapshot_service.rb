class ApkSnapshotService
  ...
  class << self
    ...
    def run_new
      batch = Sidekiq::Batch.new
      size = weekly_new_apps.count
      notes = "Weekly DL (New) #{Time.now.strftime("%m/%d/%Y")}"
      GoogleAccount.where(blocked: false, scrape_type: :full)
      j = ApkSnapshotJob.create!(notes: notes, job_type: :mass)
      batch.description = "Weekly Android SDK DL (New): ~#{size} APKs"
      batch.on(:complete, "ApkSnapshotService#on_complete_run_new", 'apk_snapshot_job_id' => j.id, 'apps_queued' => size)
      batch_size = 10e3.to_i
      weekly_new_apps.find_in_batches(batch_size: batch_size).with_index do |the_batch, index|
        batch.jobs do
          li "App #{index*batch_size}"
          args = the_batch.map{ |android_app| [j.id, batch.bid, android_app.id] }
          Sidekiq::Client.push_bulk('class' => ApkSnapshotServiceWorker, 'args' => args)
        end  
      end
    ...
    def run_recently_updated
      ...
      batch.on(:complete, "ApkSnapshotService#on_complete_run_recently_released", 'apk_snapshot_job_id' => j.id, 'apps_queued' => size)
      batch_size = 10e3.to_i
  ...
  def on_complete_run_new(status, options)
    ...
    Slackiq.notify(webhook_name: :main, status: status, title: "Weekly Android App Download complete (New)", 'Apps Downloaded' => apps_downloaded, 'Apps Queued' => apps_queued, 'apk_snapshot_job_id' => apk_snapshot_job_id)
  end
