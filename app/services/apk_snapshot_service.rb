    ...
    def run_n(notes, size = 10)
      ...
      if !workers
        ...
        batch.jobs do
          ...
          j = ApkSnapshotJob.create!(notes: notes)
          AndroidApp.where(newest_apk_snapshot_id: nil, mobile_priority: :high).joins(:newest_android_app_snapshot).where("android_app_snapshots.price = 0 AND android_app_snapshots.apk_access_forbidden IS NOT true").limit(size).each.with_index do |app, index|
            li "app #{index}"
