class SalesforceExportService
  ...
  def export(app: nil, mapping: nil, object_id: nil, publisher: nil, export_apps: true)
    ...
    SalesforceLogger.new('Enrichment', @account, publisher, @model_name, object_id.blank?).send!
  ...
  def data_fields(app: nil, publisher: nil)
    ...
    if publisher
      ...
      when 'ios'
        ...
        fields[IOS_SDKS_COUNT][:data] = publisher.tagged_sdk_summary[:installed_sdks].count
      when 'android'
        ...
        fields[ANDROID_SDKS_COUNT][:data] = publisher.tagged_sdk_summary[:installed_sdks].count
      end
