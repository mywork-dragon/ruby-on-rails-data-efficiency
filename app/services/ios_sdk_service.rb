  ...
  class << self
    ...
    def get_sdk_response(ios_app_id)
      ...
      if !snap.nil?
        ...
        first_snaps_with_current_sdks = IpaSnapshot.joins(:ios_sdks_ipa_snapshots).select('min(ipa_snapshots.created_at) as first_seen', :version, :ios_app_id, 'ios_sdk_id').where(id: app.ipa_snapshots.scanned, 'ios_sdks_ipa_snapshots.ios_sdk_id' => installed_sdks.pluck(:id)).group('ios_sdk_id')
        ...
        # get store snapshots
        store_snaps = IosAppSnapshot.select(:version, :released).where(ios_app_id: ios_app_id).where('released < ?', [max_current_date, max_last_date].compact.max).order(released: :desc)
