class AppsIndex < Chewy::Index
  ...
  define_type IosAppSnapshot do
    field :name, :seller, :seller_url, :copywright
  ...
  define_type AndroidAppSnapshot do
    field :name, :seller, :seller_url
    # field :seller_url, value: ->(url) {UrlHelper.url_with_domain_only(url)}
    field :company_name, value: ->(android_app_snapshot) {!android_app_snapshot.android_app.get_company.nil? ? android_app_snapshot.android_app.get_company.name : ''}
    # Seller URL
    # Seller
  end
